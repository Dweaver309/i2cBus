...
public class OLED
{
    ...
    public static void Initialize(I2CDevice.Configuration Config)
    {
        ...
        Command((int)Cmds.DisplayOff);
        Command((int)Cmds.DisplayClockDiv);
        Command((int)Cmds.DisplayRatio);
        Command((int)Cmds.Multiplex);
        Command(Height - 1);
        Command((int)Cmds.DisplayOffSet);
        Command((int)Cmds.NoOffSet);
        Command((int)Cmds.StartLine);
        Command((int)Cmds.ChargePump);
        Command((int)Cmds.VCCState);
        Command((int)Cmds.MemoryMode);
        Command((int)Cmds.LowColumn);
        Command((int)Cmds.SegRemap);
        Command((int)Cmds.ComScanDec);
        Command((int)Cmds.SetComPins);
        Command((int)Cmds.DisableLRRemap);
        Command((int)Cmds.SetContrast);
        Command((int)Cmds.NoExternalVcc);
        Command((int)Cmds.PreCharge);
        Command((int)Cmds.InternalDC);
        Command((int)Cmds.ComDetect);
        Command((int)Cmds.SetComDetect);
        Command((int)Cmds.DisplayResume);
        Command((int)Cmds.NormalDisplay);
        Command((int)Cmds.DeactivateScroll);
        Command((int)Cmds.DisplayOn);
    }
    ...
    ///     ''' </summary>
    private static void DrawCharacter(int Horizonal, int Line, int ASCII)
    {
        for (int i = 0; i <= 4; i++)
        {
    ...
    ///     ''' </summary>
    public static void Write(int Horizonal, int Line, string Str, bool Center = false)
    {
        if (Center)
        ...
        {
            int Asc = Microsoft.VisualBasic.Strings.AscW(Str.Substring(Counter, 1));
    ...
    public static void WriteToDevice(byte[] WriteBuffer, int TransactionTimeout = 1000)
    {
        try
        {
            ...
            I2CDevice.I2CTransaction[] XActions = new I2CDevice.I2CTransaction[] { I2CDevice.CreateWriteTransaction(WriteBuffer) };

            // the i2c data is sent here to the device.
            int transferred = I2cBus.Device.Execute(XActions, TransactionTimeout);
